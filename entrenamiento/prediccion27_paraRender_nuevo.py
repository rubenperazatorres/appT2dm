...
print("********** Sistema ANFIS - Predicción de Diabetes Tipo 2 ver. par render.com **********")
...
# Cargar el dataset
ruta = r"C:\python\mi_proyecto\entrenamiento\dataSet.csv"
...
for epoch in range(epocas_adam):
    ...
    #print("Shape salida del modelo:", y_pred.shape)
    #print("Shape del target:", y_train_tensor.shape)    
    ...
    if epoch % 100 == 0 or epoch == epocas_nadam_inicio-1:
        print(f"Epoch (Adam): {epoch}, Loss: {loss.item():.4f}, Accuracy: {accuracy:.2f}%, LR: {optimizer_adam.param_groups[0]['lr']:.6f}")
...
for epoch in range(epocas_nadam_inicio, epocas_nadam_fin):
    ...
    if epoch % 100 == 0 or epoch == epocas_nadam_fin:
        ...
        print(f"Epoch (Nadam): {epoch}, Loss: {loss.item():.4f}, Accuracy: {accuracy.item()*100:.2f}%, LR: {optimizer_nadam.param_groups[0]['lr']:.6f}")
...
scaler_params = {
    "mean_": scaler.mean_.tolist(),        # Media de cada feature
    "scale_": scaler.scale_.tolist(),      # Desviación estándar
    "var_": scaler.var_.tolist(),          # Varianza
    "n_features_in_": scaler.n_features_in_,  # Número de features
    "feature_names_in_": scaler.feature_names_in_.tolist()  # Nombres de columnas (opcional)
}
